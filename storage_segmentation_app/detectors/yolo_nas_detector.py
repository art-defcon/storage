...
class YOLONASDetector(BaseDetector):
    ...
    using YOLO-NAS for improved accuracy (10-17% higher mAP than YOLOv8).
    """
    
    def __init__(self, confidence_threshold=0.5, model_size="l"):
        """
        Initialize the detector with YOLO-NAS model and parameters.
        ...
        if self.model_size not in ['s', 'm', 'l']:
        ...
        model_filename = f"yolo_nas_{self.model_size}.pt"
        ...
        if root_model_path.exists():
            ...
            # Use the default YOLO-NAS model from Ultralytics
            self.model_path = f"yolo_nas_{self.model_size}.pt"
        
        # Load models
        self._load_models()
    
    def _load_models(self):
        """Load the YOLO-NAS model for improved detection accuracy."""
        try:
            # Load YOLO-NAS model
            self.model = YOLO(self.model_path)
            print(f"Loaded YOLO-NAS-{self.model_size.upper()} model from {self.model_path}")
    ...
    def process_image(self, image, detect_compartments=True, filter_small_segments=False, min_segment_width=None):
        """
        Process an image to detect storage units and their compartments using YOLO-NAS.
        ...
        # Detect storage units with YOLO-NAS
        ...
        for result in unit_results:
            ...
            for i, box in enumerate(boxes):
                ...
                print(f"YOLO-NAS detected: {class_name} (class ID: {class_id}) with confidence {confidence:.2f}")
    ...
    def _detect_compartments(self, image, storage_unit, filter_small_segments=False, min_segment_width=None):
        """
        Detect compartments within a storage unit using YOLO-NAS.
        ...
        # Detect compartments in the resized unit image using YOLO-NAS
        ...
        for result in compartment_results:
            ...
            for i, box in enumerate(boxes):
                ...
                # Print detection info for debugging
                print(f"YOLO-NAS detected compartment: {class_name} (class ID: {class_id}) with confidence {confidence:.2f}")
    ...
    def _get_all_segments(self, image):
        """
        Get all possible segments from YOLO-NAS without any filtering.
        ...
        # Detect all segments with YOLO-NAS
        ...
        for result in results:
            ...
            for i, box in enumerate(boxes):
                ...
                # Get the actual class name from YOLO model
                class_name = classes.get(class_id, f"Unknown-{class_id}")
                
                # Print detection info for debugging
                print(f"Raw YOLO-NAS segment detected: {class_name} (class ID: {class_id}) with confidence {confidence:.2f}")
                ...
                segment = StorageUnit(
                    ...
                    confidence=confidence,
                    class_id=class_id,
                    class_name=f"YOLO-NAS {class_name}",  # Prefix with model name
                    mask=mask
                )
                
                segments.append(segment)
        
        print(f"Total raw segments detected by YOLO-NAS: {len(segments)}")
        return segments